...
class KeywordSet(object):
    ...
    @property
    def title(self):
        if self.top_level and not self._title:
            ident = self.find('STANDIDENT')
...
#             s += '%s\n' % keyword.__str__()  # .strip()
    ...
    def find(self, kwd):
        ...
        for item in self.items:
            ...
            elif isinstance(item, KeywordBase) and item.children:
                for child in item.children:
                    if isinstance(child, KeywordSet):
                        r = child.find(kwd)
                        objs.extend(r)
                        continue

                    if child.mnemonic == kwd:
                        objs.append(child)
